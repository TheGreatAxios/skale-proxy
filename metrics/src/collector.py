...
from src.db import update_transaction_counts
from src.utils import transform_to_dict
...
from src.types import AddressCounter, AddressCountersMap
...
async def get_address_counters(
    ...
    for attempt in range(API_ERROR_RETRIES):
        try:
            return await fetch_address_data(session, url)
...
async def collect_metrics(network_name: str):
